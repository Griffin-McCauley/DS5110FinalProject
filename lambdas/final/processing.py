...
def lambda_handler(event, context):
    try:
        inbucket_name = event["inbucket"]
        outbucket_name = event["outbucket"]
        file = event["file"]
        infile_name = f'raw/{file}'
        startread = datetime.now(pytz.timezone('US/Eastern')).timestamp()
        resp = s3_resource.Object(inbucket_name, infile_name).get()
        ...
        for i, type in enumerate(["training","validation","testing"]):
            ...
            s3_resource.Object(outbucket_name, outfile_name).put(Body=splits[i].to_csv())
            
        string = f'{(stopread-startread)*1000}'
        encoded_string = string.encode("utf-8")
        
        timefile_name = f'processed/runtimes/reads3.txt'
        s3_resource.Object(outbucket_name, timefile_name).put(Body=encoded_string)
